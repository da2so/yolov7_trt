...
class Detect(nn.Module):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                if self.grid[i].shape[2:4] != x[i].shape[2:4]:
                    self.grid[i], anchor_grid= self._make_grid(nx, ny, x[i].device, i)
    ...
    def _make_grid(self, nx=20, ny=20, device='cpu', i=0):
        yv, xv = torch.meshgrid([torch.arange(ny, device=device), torch.arange(nx, device=device)])
        grid = torch.stack((xv, yv), 2).view((1, 1, ny, nx, 2)).float()
        anchor_grid = (self.anchors[i] * self.stride[i]).view((1, self.na, 1, 1, 2)).float()
